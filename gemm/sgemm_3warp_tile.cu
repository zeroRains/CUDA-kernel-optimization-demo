    ...
    sgemm_warptiling_kernel(float *A, float *B, float *C, int M, int N, int K)
{
    ...
    const uint warp_idx = threadIdx.x / WARPSIZE; // 第几个warp
    const uint warp_col = warp_idx % (BN / WN);   // warp在block中的列坐标
    const uint warp_row = warp_idx / (BN / WN);   // warp在block中的行坐标

    // warp tile 的大小
    // WM 是每个 Warp 处理数据的行数，WN 是每个 Warp 处理数据的列数 -> 一个warp处理32*32的矩阵
    // 数据行数 / 迭代次数 = 每次迭代处理的行数
    constexpr uint WSUBM = WM / WMITER; // 每次处理M的行数32
    constexpr uint WSUBN = WN / WNITER; // 每次处理N的行数16

    // warp 内的线程索引
    const uint thread_idx_in_warp = threadIdx.x % WARPSIZE;            // [0, 31] 4*4矩阵的索引
    const uint thread_col_in_warp = thread_idx_in_warp % (WSUBN / TN); // 线程内列索引
    const uint thread_row_in_warp = thread_idx_in_warp / (WSUBN / TN); // 线程内行索引
    ...
    // 和上个版本一样
    // ldg = BK*BM/thread/4 ->stride_a = BM/lkg = BM/BK/BM*thread*4 = 4*thread/BK
    const int stride_a = (NUM_THREADS * 4) / BK;
    // 一共 BN 行，搬运 ldg_b_num 轮，每轮搬运 stride_b 行
    const int stride_b = (NUM_THREADS * 4) / BN;
    ...
    A += c_row * BM * K;
    B += c_col * BN;
    C += (c_row * BM + warp_row * WM) * N + c_col * BN + warp_col * WN; // 多一个warp路径
