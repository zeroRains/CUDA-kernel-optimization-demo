...
__global__ void __launch_bounds__(NUM_THREADS)
    sgemm_double_buffering_kernel(float *A, float *B, float *C, int M, int N, int K)
{
    ...
    for (uint bk_idx = 0; bk_idx < K; bk_idx += BK)
    {
        ...
        for (uint offset = 0; offset < BM; offset += stride_a)
        {
            ...
            smem_a[write_index][OFFSET(inner_col_a, offset + inner_row_a, BM)] = tmp.x;
            smem_a[write_index][OFFSET(inner_col_a + 1, offset + inner_row_a, BM)] = tmp.y;
            smem_a[write_index][OFFSET(inner_col_a + 2, offset + inner_row_a, BM)] = tmp.z;
            smem_a[write_index][OFFSET(inner_col_a + 3, offset + inner_row_a, BM)] = tmp.w;
        }
        ...
        for (uint offset = 0; offset < BK; offset += stride_b)
        {
            ...
            FETCH_FLOAT4(smem_b[write_index][OFFSET(inner_row_b + offset, inner_col_b, BN)]) =
        ...
        for (uint dot_idx = 0; dot_idx < BK; ++dot_idx)
        {
            for (uint warp_sub_row_idx = 0; warp_sub_row_idx < WMITER; ++warp_sub_row_idx)
            {
                for (int m = 0; m < TM; m += 4)
                {
                    ...
                    FETCH_FLOAT4(reg_a[write_index][warp_sub_row_idx * TM + m]) = FETCH_FLOAT4(
                        smem_a[write_index][OFFSET(dot_idx, warp_row * WM + warp_sub_row_idx * WSUBM + thread_row_in_warp * TM + m, BM)]);
                }
            }
            for (uint warp_sub_col_idx = 0; warp_sub_col_idx < WNITER; ++warp_sub_col_idx)
            {
                for (int n = 0; n < TN; n += 4)
                {
                    ...
                    FETCH_FLOAT4(reg_b[write_index][warp_sub_col_idx * TN + n]) = FETCH_FLOAT4(
                        smem_b[write_index][OFFSET(dot_idx, warp_col * WN + warp_sub_col_idx * WSUBN + thread_col_in_warp * TN + n, BN)]);
                }
            }
                ...
                for (uint warp_sub_col_idx = 0; warp_sub_col_idx < WNITER; ++warp_sub_col_idx)
                {
                    for (int m = 0; m < TM; m++)
                    {
                        for (int n = 0; n < TN; n++)
                        {
                            ...
                            thread_results[(warp_sub_row_idx * TM + m) * (WNITER * TN) + (warp_sub_col_idx * TN) + n] += reg_a[write_index][warp_sub_row_idx * TM + m] * reg_b[write_index][warp_sub_col_idx * TN + n];
                        }
                    }
                }
            }
...
void run_sgemm_double_buffering(float *A, float *B, float *C, int m, int n, int k)
{
    ...
    sgemm_double_buffering_kernel<BM, BN, BK, WM, WN, WMITER, WNITER, TM,
                                  TN, NUM_THREADS>
...
int main(int argc, char *argv[])
{
    ...
    run_sgemm_double_buffering(d_A, d_B, d_C, m, n, k);
